...
# leitura -----------
ipca <- window(readRDS("data/ipca.rds"), start = c(2003,7), freq = 12)
...
dados <- data.frame(window(cbind(ipca,dummy),start = start(ipca), freq = 12))
...
initial_gamma <- c(0.08764706,-0.02529412,-0.11764706,-0.11411765,-0.22882353,-0.42529412,-0.26941176,-0.35411765,-0.29882353,-0.14352941,-0.02588235)
...
dcs1_normal <- dcs_fk_estimation(ipca, initial = parametros1_normal, type = "BSM1_normal", outlier = F, otimo = T)
...
ts.plot(ipca,dcs1_normal$out[,"mu"], col = 1:2)
...
parametros1_normald <- list(
  ...
  Dummy = cbind(d1 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2002,11)))
)
parametros1_normald

dcs1_normald <- dcs_fk_estimation(ipca, initial = parametros1_normald, type = "BSM1_normal", outlier = T, otimo = T)
...
ts.plot(ipca,dcs1_normald$out[,"mu"], col = 1:2)
...
dcs2_normal <- dcs_fk_estimation(ipca, initial = parametros2_normal, type = "BSM2_normal", outlier = F, otimo = T)
...
ts.plot(ipca,dcs2_normal$out[,"mu"], col = 1:2)
...
dcs2_normald <- dcs_fk_estimation(ipca, initial = parametros2_normald, type = "BSM2_normal", outlier = T, otimo = T)
...
ts.plot(ipca,dcs2_normald$out[,"mu"], col = 1:2)
...
dcs1 <- dcs_fk_estimation(ipca, initial = parametros1, type = "BSM1", outlier = F, otimo = T)
...
ts.plot(ipca,dcs1$out[,"mu"], col = 1:2)
...
dcs1_d <- dcs_fk_estimation(ipca, initial = parametros1_d, type = "BSM1", outlier = T, otimo = T)
...
ts.plot(ipca,dcs1_d$out[,"mu"], col = 1:2)
...
dcs2 <- dcs_fk_estimation(ipca, initial = parametros2, type = "BSM2", outlier = F, otimo = T)
...
ts.plot(ipca,dcs2$out[,"mu"], col = 1:2)
...
dcs2_d <- dcs_fk_estimation(ipca, initial = parametros2_d, type = "BSM2", outlier = T, otimo = T)
...
ts.plot(ipca,dcs2_d$out[,"mu"], col = 1:2)
...
dcs3_normal <- dcs_fk_estimation(ipca, initial = parametros3_normal, type = "BSM3_normal", outlier = F, otimo = T)
...
ts.plot(ipca,dcs3_normal$out[,"mu"], col = 1:2)
...
dcs3_normald <- dcs_fk_estimation(ipca, initial = parametros3_normald, type = "BSM3_normal", outlier = T, otimo = T)
...
ts.plot(ipca,dcs3_normald$out[,"mu"], col = 1:2)
...
dcs3 <- dcs_fk_estimation(ipca, initial = parametros3, type = "BSM3", outlier = F, otimo = T)
...
parametros3_d <- list(
  par = data.frame(
    name =  c("k1","k2","ks","f2","df","beta","mu[1|0]",paste0("gamma",1:11)          ,"psi","phi","k3","d1","d2","d3","d4"),
    value = c(0.3 ,0   ,0.5 ,5   ,8   ,0     ,0.5        ,as.vector(initial_gamma)[1:11],-1    ,0.6  ,0.5 ,0, 0 ,0,0),
    lower = c(0.0 ,0   ,0.0,-Inf,4   ,0     ,-Inf     ,rep(-Inf,11)                  ,-Inf ,-1   ,0   ,-Inf,-Inf,-Inf,-Inf),
    upper = c(Inf ,0   ,Inf ,Inf ,Inf ,0     ,Inf      ,rep(Inf,11)                   ,Inf  ,1    ,Inf ,Inf,Inf,Inf,Inf)
  ),
  Dummy = cbind(d1 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2002,11)),
                d2 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2001,7)),
                d3 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2002,12)),
                #d4 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2002,7)),
                d4 = BETS.dummy(start = start(ipca), end = end(ipca), freq = 12, date = c(2003,1)))
)
parametros3_d

dcs3_d <- dcs_fk_estimation(ipca, initial = parametros3_d, type = "BSM3", outlier = T, otimo = T)
