...
nucleos <- window(cbind(nucleo_tf[,3],nucleo_dcs, nucleo_comb), start = c(2001,1), end = c(2017,12), freq = 12)
colnames(nucleos) <- c("S","DCS","COMB")
...
lines(nucleos[,"COMB"], lwd = 3, lty = 1, col = "forestgreen")
abline(h = seq(0.2,1.2,0.2), col = "#C9C9C9", lty = 3)
abline(v = 1999:2018, col = "#C9C9C9", lty = 3)
#abline(h = 0, col = "#CC3232", lty = 2)
legend(2005.2,1.2, legend = c("Núcleo-S","Núcleo-DCS","Núcleo-COMB"), lwd = c(2,2,3), lty = c(5,1,1), y.intersp = 1.5,
       col = c("#1874CD","#CD0000","forestgreen"), cex = 1,bg = "white", box.col = "white",box.lwd = 0)
...
abline(v = 1999:2018, col = "#C9C9C9", lty = 3)
#abline(h = 0, col = "#CC3232", lty = 2)
legend(2005,3.5, legend = c("IPC-Br","Núcleo-COMB"), lwd = c(1,2), lty = c(4,1), y.intersp = 1.5,
       col = c(1,"forestgreen"), cex = 1.3,bg = "white", box.col = "white",box.lwd = 0)
...
bias_full <- rbind(core.diag(ipc, nucleos[,"S"], test = "bias")$out$full,
                   ...
                   core.diag(ipc, nucleos[,"COMB"], test = "bias")$out$full[2,])
bias_part1 <- rbind(core.diag(ipc, nucleos[,"S"], test = "bias")$out$part1,
                    core.diag(ipc, nucleos[,"DCS"], test = "bias")$out$part1[2,],
                    core.diag(ipc, nucleos[,"COMB"], test = "bias")$out$part1[2,])
bias_part2 <- rbind(core.diag(ipc, nucleos[,"S"], test = "bias")$out$part2,
                    core.diag(ipc, nucleos[,"DCS"], test = "bias")$out$part2[2,],
                    core.diag(ipc, nucleos[,"COMB"], test = "bias")$out$part2[2,])

rownames(bias_full) = rownames(bias_part1) = rownames(bias_part2) <- c("IPC-Br", "Núcleo-S", "Núcleo-DCS","Núcleo-COMB")
...
coint <- core.diag(ipc, nucleos[,"COMB"], test = "coint")
...
atratividade <- core.diag(ipc, nucleos[,"COMB"], test = "attract", 
                          lags_y = c(12), 
                          lags_core = c(9,12))
...
atratividade <- core.diag(ipc0, nucleos0[,"COMB"], test = "attract", 
                          lags_y = c(12),
                          lags_core = c(2,6,8))
...
# nucleo-comb
fcst1 <- core.fcst(ipc,  nucleos[,"COMB"], lag = 1, h = 12)
fcst3 <- core.fcst(ipc,  nucleos[,"COMB"], lag = 3, h = 12)
fcst6 <- core.fcst(ipc,  nucleos[,"COMB"], lag = 6, h = 12)
fcst12 <- core.fcst(ipc, nucleos[,"COMB"], lag = 12, h = 12)
...
# nucleo-dcs
fcst1 <- core.fcst(ipc0,  nucleos0[,"COMB"], lag = 1, h = 12)
fcst3 <- core.fcst(ipc0,  nucleos0[,"COMB"], lag = 3, h = 12)
fcst6 <- core.fcst(ipc0,  nucleos0[,"COMB"], lag = 6, h = 12)
fcst12 <- core.fcst(ipc0, nucleos0[,"COMB"], lag = 12, h = 12)
