...
# estabilidade
#estabilidade <- dcs_estabilidade(y = ipc, start = c(2016,1), initial = parametros3_d, type = "BSM3", outlier = T)
#saveRDS(estabilidade, "data/estabilidade_dcs_t.rds")
estabilidade <- readRDS("data/estabilidade_dcs_t.rds")
...
#                  series.modelspan = "2009.jan,2015.dec")
# 
#   ajuste$series$s11
# 
# }
...
polygon(c(2016 + 0.0833*5,2016 + 0.0833*6,2016 + 0.0833*6,2016 + 0.0833*5),c(0.2219,0.2219,0.923,0.923), col = "#B2DFEE", border = NA)
polygon(c(2016 + 0.0833*10,2016 + 0.0833*11,2016 + 0.0833*11,2016 + 0.0833*10),c(0.2219,0.2219,0.923,0.923), col = "#B2DFEE", border = NA)
polygon(c(2016 + 0.0833*15,2016 + 0.0833*16,2016 + 0.0833*16,2016 + 0.0833*15),c(0.2219,0.2219,0.923,0.923), col = "#B2DFEE", border = NA)
polygon(c(2016 + 0.0833*20,2016 + 0.0833*21,2016 + 0.0833*21,2016 + 0.0833*20),c(0.2219,0.2219,0.923,0.923), col = "#B2DFEE", border = NA)
polygon(c(2016 + 0.0833*22,2016 + 0.0833*23,2016 + 0.0833*23,2016 + 0.0833*22),c(0.2219,0.2219,0.923,0.923), col = "#B2DFEE", border = NA)
lines(estabilidade_tf$out[,2], col = "#1874CD", lty = 1, lwd = 2)
lines(estabilidade_tf$out[,1], col = "#1874CD", lty = 5, lwd = 1)
...
legend(2017.08,0.923, legend = c("mês a mês","completo"), lwd = c(1,2), lty = c(5,1),# y.intersp = 1.5,
...
abline(v = seq(2016,2018,0.0833), h = seq(0.3,0.9,0.1),lty = 3, col = "#C9C9C9")
axis(1, at = seq(2016,2018,0.0833*3) , labels = substr(as.Date(estabilidade_tf$out),1,7)[seq(1,25,3)])
legend(2017.073,0.923, legend = c("mês a mês","completo"), lwd = c(1,2), lty = c(5,1),# y.intersp = 1.5,
       col = c("#CD0000","#CD0000"), cex = 1.1, bg = "white", box.col = "white", box.lwd = 0)

round(sqrt(mean((estabilidade$out[,1] - estabilidade$out[,2])^2)),2)
round(sqrt(mean((estabilidade_tf$out[,1] - estabilidade_tf$out[,2])^2)),2)
