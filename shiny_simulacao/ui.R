shinyUI(
  navbarPage(theme = shinytheme("lumen"), shinyjs::useShinyjs(),
             ...
             tabPanel("IPC",
                      ...
                      fluidRow(
                        column(10, offset = 1, dataTableOutput("ipc_dcs_parametros", width = "100%"))
                      ), hr(),
                      
                      fluidRow(
                        column(6,
                               div("DCS (sem psi sem dummy)", style = "text-align:center; background-color:#FFEEE5; font-weight:bold"),
                               plotOutput("dcs_padrao_plot1"),
                               plotOutput("dcs_padrao_plot2"),
                               plotOutput("dcs_padrao_plot3"),
                               plotOutput("dcs_padrao_plot4", height = "230px"),
                               fluidRow(
                                 column(10, offset = 1, dataTableOutput("ipc_dcs_padrao_stats", width = "90%"))
                               )
                        ),
                        column(6,
                               div("DCS (sem psi com dummy)", style = "text-align:center; background-color:#FFEEE5; font-weight:bold"),
                               plotOutput("dcs_padrao_dummy_plot1"),
                               plotOutput("dcs_padrao_dummy_plot2"),
                               plotOutput("dcs_padrao_dummy_plot3"),
                               plotOutput("dcs_padrao_dummy_plot4", height = "230px"),
                               fluidRow(
                                 column(10, offset = 1, dataTableOutput("ipc_dcs_padrao_dummy_stats", width = "90%"))
                               )
                        )
                      ), hr(),
                      fluidRow(
                        column(6,
                               div("DCS (com psi sem dummy)", style = "text-align:center; background-color:#E5F7FF; font-weight:bold"),
                               plotOutput("dcs_padrao_psi_plot1"),
                               plotOutput("dcs_padrao_psi_plot2"),
                               plotOutput("dcs_padrao_psi_plot3"),
                               plotOutput("dcs_padrao_psi_plot4", height = "230px"),
                               fluidRow(
                                 column(10, offset = 1, dataTableOutput("ipc_dcs_padrao_psi_stats", width = "90%"))
                               )
                        ),
                        column(6,
                               div("DCS (com psi com dummy)", style = "text-align:center; background-color:#E5F7FF; font-weight:bold"),
                               plotOutput("dcs_padrao_psi_dummy_plot1"),
                               plotOutput("dcs_padrao_psi_dummy_plot2"),
                               plotOutput("dcs_padrao_psi_dummy_plot3"),
                               plotOutput("dcs_padrao_psi_dummy_plot4", height = "230px"),
                               fluidRow(
                                 column(10, offset = 1, dataTableOutput("ipc_dcs_padrao_psi_dummy_stats", width = "90%"))
                               )
                        )
                      )
             ...
             tabPanel("Comparar Tendências",
                      
                      sidebarLayout(
                        sidebarPanel(width = 3,
                          checkboxGroupInput(
                            inputId = "tendencias", label = div("OPÇÕES:", style = "font-size:110%;color:#104E8B;font-weight:bold"),
                            choices = c("IPC","IPC-MA","IPC - DCS (sem psi sem dummy)",
                                        "IPC - DCS (sem psi com dummy)",
                                        "IPC - DCS (com psi sem dummy)",
                                        "IPC - DCS (com psi com dummy)",
                                        "IPC-MA - DCS (sem psi sem dummy)",
                                        "IPC-MA - DCS (sem psi com dummy)",
                                        "IPC-MA - DCS (com psi sem dummy)",
                                        "IPC-MA - DCS (com psi com dummy)"
                            ),selected = c("IPC-MA","IPC - DCS (com psi com dummy)") )
                        ),
                        mainPanel(
                          dygraphOutput("grafico_trend_dcs")
             ...
             tabPanel("Artigo Caivano, Harvey & Luati",
                      ...
                      fluidRow(
                        column(offset = 1, width = 10,
                               ...
                               plotOutput("turistas_plot3", height = "350px"), 
                               plotOutput("turistas_plot4", height = "350px"), hr(),
                               plotOutput("turistas_plot5", height = "480px"),
                               plotOutput("turistas_plot6", height = "250px"), hr(),
                               dataTableOutput("turistas_diag"), hr()
