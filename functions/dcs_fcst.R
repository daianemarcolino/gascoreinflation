dcs_fcst <- function(y, initial = NULL, type = "BSM2_beta_psi", outlier = F){
  ...
  out <- dcs_fk_estimation(ipc0, initial = parametros_iniciais, type = "BSM2_beta_psi", outlier = F)
  ts.plot(ipc0, out$out[,"mu"])
  
  parametros_iniciais$par$value <- out$otimizados$par
  parametros_iniciais$par$lower <- out$otimizados$par
  parametros_iniciais$par$upper <- out$otimizados$par
  ...
  for(i in 1:m){
    for(j in 1:k){
      yt[j,i] <- rt(1, df = out$otimizados$par[4])*tail(out$out[,"sigma"],1) + sum(tail(out$out[,c("mu","gamma","psi")],1))
      out <- dcs_fk_estimation(ts(c(ipc0,yt[j,i]), start = start(ipc0), freq = 12),
                               initial = parametros_iniciais, type = "BSM2_beta_psi", outlier = F)
      parametros_iniciais$par$value <- out$otimizados$par
      message("k:",j)
    }
    message("m:",i)
  ...
  ipc_prev <- ts(rowMeans(yt), start = c(2016,1), freq = 12)
  
  ts.plot(ipc0,ipc_prev, ipc)
  if(type == "BSM_artigo"){ 
    # ARTIGO -----  
    ...
    # DCS COM BETA VARIANTE NO TEMPO -----
    # modelo:
    ...
    if(outlier){
      ...
      score <- ((df + 1)/(df*exp(2*f2)))*u1
      out <- cbind(mu, gamma, f2, exp(f2), epsilon, nu, score, u1, b,dum)
      colnames(out) <- c("mu","gamma","f2","sigma","epsilon", "nu", "score","u", "b","dummy")
    }else{
      for(t in 1:(N-1)){ 
        u1[t] <- (y[t] - mu[t] - gamma[t])/(1 + (y[t] - mu[t] - gamma[t])^2/(df*exp(2*f2)))
        mu[t+1] <- mu[t] + k1*u1[t]
