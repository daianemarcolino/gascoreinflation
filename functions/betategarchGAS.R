betategarchGAS <- function(y, initial = NULL, type = "mean-var"){
  ...
  if(type == "var"){
    ...
    inicializar <- function(y, par){
      ...
      p2 <- -((df+1)/2)*sum(log((1 + y^2/(df*exp(2*f2)))))
    ...
    otimizar <- function(y, par){
      ...
      f2 <- par[5]
      for(t in 1:N){
        f2[t+1] <- w1 + A1*((df + 3)/(2*df) * (((df + 1)*((y[t] - 0)^2/(df*exp(2*f2[t])))/(1 + (y[t] - 0)^2/(df*exp(2*f2[t])))) - 1)) + B1*f2[t]
      }
    ...
    otimizados <- nlminb(start = c(0.02,0.05,0.95, ci$par), objective = otimizar, y = y, #ci = ci,
     lower = c(-Inf,-Inf,-1,2,-Inf), upper = c(Inf,Inf,1,Inf,Inf))
