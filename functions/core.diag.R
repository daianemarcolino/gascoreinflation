core.diag <- function(y, core, par, conf = 0.95){
  ...
  diag2$conclusion <- ifelse(diag2$stats > diag2$t_value, "reject H0", "don't reject H0")
  ...
  modelo_core <- dynlm(difs[,"core"] ~ difs[,"erro_1"] + L(difs[,"core"],1) +
                         #L(difs[,"core"],2) + 
                         L(difs[,"core"],3) +
                         #L(difs[,"core"],4) + 
                         L(difs[,"core"],5) +
