...
prevs <- round(window(cbind(ipc,prev_1frente,prev_3frente,prev_6frente,prev_12frente), start= c(2017,1), freq = 12),2)
prevs2 <- prevs

library(plotrix)
barp(sqrt(colMeans((ipc - prevs2[,2:4])^2)))

plot(prevs2[,1], col = 1, lwd = 1, lty = 3, ylim = c(-0.5,1),
     xaxt = "n", ylab = "variação mensal percentual (%)", xlab = "", type = "o")
lines(prevs2[,2], col = "#1874CD", lwd = 1, lty = 1, type = "o")
lines(prevs2[,3], col = "#1874CD", lwd = 2, lty = 1, type = "o")
lines(prevs2[,4], col = "#CD0000", lwd = 2, lty = 4, type = "o")
abline(v = seq(2016,2018,0.0833), h = seq(-0.5,1,0.25),lty = 3, col = "#C9C9C9")
axis(1, at = seq(2017.417,2018,0.0833) , labels = substr(as.Date(prevs2),1,7))
legend(2017.6,1, legend = c("IPC-Br",1,3,6), lwd = c(1,1,2,2), lty = c(3,1,1,4),# y.intersp = 1.5,
       col = c(1,"#1874CD","#1874CD","#CD0000"), cex = 1.1,bg = "white", box.col = "white",box.lwd = 0)

sqrt(mean((ipc - prevs2[,2])^2))
sqrt(mean((ipc - prevs2[,3])^2, na.rm = T))
sqrt(mean((ipc - prevs2[,4])^2, na.rm = T))
sqrt(mean((ipc - prevs2[,5])^2, na.rm = T))
